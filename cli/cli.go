...
func New(handlers ...Handler) *Cli {
	...
	// in order to handle `openebs help` appropriately
	cli := new(Cli)
...
func (cli *Cli) noSuchCommand(command string) {
	...
	fmt.Fprintf(cli.Stderr, "openebs: '%s' is not a openebs command.\nSee 'openebs --help'.\n", command)
	os.Exit(1)
}
...
// CmdHelp displays information on a OpenEBS command.
//
// If more than one command is specified, information is only shown for the first command.
//
// Usage: openebs help COMMAND or openebs COMMAND --help
...
// Subcmd is a subcommand of the main "openebs" command.
// A subcommand represents an action that can be performed
// from the OpenEBS command line client.
//
// To see all available subcommands, run "openebs --help".
func Subcmd(name string, synopses []string, description string, exitOnError bool) *flag.FlagSet {
	...
	flags.ShortUsage = func() {
		...
		for i, synopsis := range synopses {
			...
			fmt.Fprintf(flags.Out(), "\n%sopenebs %s%s", lead, name, synopsis)
		}
