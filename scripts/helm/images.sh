...
# The upgrade-job is currently only deployed on-demand by the kubectl-plugin
# Until such time we integrate the upgrade-job into the helm chart, we need to extract the image
# from the images.txt file which is generated by the helm chart.
# The upgrade-job image is derived from the callhome image (any from extensions would be fine)
upgrade_job_image() {
  local images="${1:-}"
  local registry="${OPENEBS_REGISTRY:-docker.io}"
  local namespace="${OPENEBS_NAMESPACE:-openebs}"
  local tag

  # Remove the mayastor-upgrade-job because we have an umbrella-specific upgrade-job instead
  sed -i '/\/mayastor-upgrade-job:/d' "$images"

  tag="$(yq -r ".version" "$CHART")"

  # ie docker.io/openebs/openebs-upgrade-job:v4.6.0
  echo "${registry%/}/${namespace%/}/openebs-upgrade-job:v${tag#v}" >> "$images"
}
...
Options:
  ...
  --openebs-registry <reg>   Registry used to build the openebs-upgrade-job image.
                             Defaults to the registry of the callhome image.
  --openebs-namespace <ns>   Namespace/repository used to build the openebs-upgrade-job image.
                             Defaults to the namespace of the callhome image.
...
while test $# -gt 0; do
  ...
  case "$arg" in
    ...
    --openebs-registry)
      shift
      [ $# -eq 0 ] && log_fatal "--openebs-registry requires an argument"
      OPENEBS_REGISTRY="$1"
      ;;
    --openebs-namespace)
      shift
      [ $# -eq 0 ] && log_fatal "--openebs-namespace requires an argument"
      OPENEBS_NAMESPACE="$1"
      ;;
    *)
