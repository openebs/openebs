...
func TestParseHost(t *testing.T) {
	...
	valid := map[string]string{
		...
		"tcp://openebs.com:9440":    "tcp://openebs.com:9440",
...
func TestParseOpenEBSDaemonHost(t *testing.T) {
	invalids := map[string]string{
		...
		"tcp://unix:///run/openebs.sock": "Invalid proto, expected tcp: unix:///run/openebs.sock",
	...
	valids := map[string]string{
		...
		"tcp://:7777/path":        fmt.Sprintf("tcp://%s:7777/path", DefaultHTTPHost),
		"unix:///run/openebs.sock": "unix:///run/openebs.sock",
		"unix://":                 "unix://" + DefaultUnixSocket,
		"fd://":                   "fd://",
		"fd://something":          "fd://something",
	...
	for invalidAddr, expectedError := range invalids {
		if addr, err := parseOpenEBSDaemonHost(invalidAddr); err == nil || err.Error() != expectedError {
			t.Errorf("tcp %v address expected error %q return, got %q and addr %v", invalidAddr, expectedError, err, addr)
		}
	}
	for validAddr, expectedAddr := range valids {
		if addr, err := parseOpenEBSDaemonHost(validAddr); err != nil || addr != expectedAddr {
...
func TestParseInvalidUnixAddrInvalid(t *testing.T) {
	if _, err := parseSimpleProtoAddr("unix", "tcp://127.0.0.1", "unix:///var/run/openebs.sock"); err == nil || err.Error() != "Invalid proto, expected unix: tcp://127.0.0.1" {
		t.Fatalf("Expected an error, got %v", err)
	}
	if _, err := parseSimpleProtoAddr("unix", "unix://tcp://127.0.0.1", "/var/run/openebs.sock"); err == nil || err.Error() != "Invalid proto, expected unix: tcp://127.0.0.1" {
		t.Fatalf("Expected an error, got %v", err)
	}
	if v, err := parseSimpleProtoAddr("unix", "", "/var/run/openebs.sock"); err != nil || v != "unix:///var/run/openebs.sock" {
		t.Fatalf("Expected an %v, got %v", v, "unix:///var/run/openebs.sock")
	}
}
