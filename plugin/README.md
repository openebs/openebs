...
While the original kubectl-openebs plugin only supported LocalPV engines, this plugin extends that support to include Mayastor and organizes commands by engine type for improved clarity and usability.

## Namespace handling
This plugin avoids the hardcoded "mayastor" namespace assumption used by the original kubectl-mayastor plugin.  Instead, if a namespace is not explicitly provided, the plugin defaults to the namespace defined in the current kubeconfig context.  This allows users to manage OpenEBS installations in any namespace. For example, if OpenEBS is installed in the "storage" namespace, users can set their current context's namespace using:

```
kubectl config set-context --namespace=storage --current
```

It's important to note that setting the context's namespace will affect how other kubectl commands resolve resources when a namespace is not explicitly specified.
...
The general command structure is `kubectl openebs <engine> <operation> <resource>` where engines can be any OpenEBS provided engine. All localpv engines has only `get` commands now.
...
8. Show pool labels.
```
❯ kubectl mayastor get pools --show-labels
...
kubectl mayastor get pool pool-1-kworker1 --show-labels
 ID               DISKS                                                     MANAGED  NODE      STATUS  CAPACITY  ALLOCATED  AVAILABLE  LABELS
pool-1-kworker1   aio:///dev/vdb?uuid=d8a36b4b-0435-4fee-bf76-f2aef980b833  true     kworker3  Online  500GiB    100GiB     400GiB  zone-us=east-1
 ```
...
```
❯ kubectl mayastor get pools --selector zone-us=east-1
 ID               DISKS                                                     MANAGED  NODE      STATUS  CAPACITY  ALLOCATED  AVAILABLE
...
❯  kubectl openebs localpv-zfs get zpools
 ...
 zfspv-pool  node-1-309787  10029612145612829144  30685394432  99840
```
...
❯  kubectl openebs localpv-zfs get zpools node-0-309787
  NAME        NODE           UUID                  FREE         USED
  zfspv-pool  node-0-309787  965947675669908193    29966204Ki   99K
 ```
