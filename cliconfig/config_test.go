...
import (
	...
	"github.com/openebs/openebs/pkg/homedir"
)
...
func TestOldValidAuth(t *testing.T) {
	...
	// defaultIndexserver is https://index.openebs.io/v1/
	ac := config.AuthConfigs["https://index.openebs.io/v1/"]
	if ac.Username != "joejoe" || ac.Password != "openebs" {
...
func TestOldJsonInvalid(t *testing.T) {
	...
	fn := filepath.Join(tmpHome, oldConfigfile)
	js := `{"https://index.openebs.io/v1/":{"auth":"test","email":"user@example.com"}}`
	if err := ioutil.WriteFile(fn, []byte(js), 0600); err != nil {
		t.Fatal(err)
	}
...
func TestOldJson(t *testing.T) {
	...
	fn := filepath.Join(tmpHome, oldConfigfile)
	js := `{"https://index.openebs.io/v1/":{"auth":"am9lam9lOmhlbGxv","email":"user@example.com"}}`
	if err := ioutil.WriteFile(fn, []byte(js), 0600); err != nil {
		t.Fatal(err)
	}
...
func TestNewJson(t *testing.T) {
	...
	fn := filepath.Join(tmpHome, ConfigFileName)
	js := ` { "auths": { "https://index.openebs.io/v1/": { "auth": "am9lam9lOmhlbGxv" } } }`
	if err := ioutil.WriteFile(fn, []byte(js), 0600); err != nil {
		t.Fatal(err)
	}
...
func TestJsonReaderNoFile(t *testing.T) {
	js := ` { "auths": { "https://index.openebs.io/v1/": { "auth": "am9lam9lOmhlbGxv", "email": "user@example.com" } } }`
