...
- Upgrade from 1.0.0 to any of 1.1.0, 1.2.0, 1.3.0, 1.4.0, 1.5.0
- Upgrade from 1.1.0 to any of 1.2.0, 1.3.0, 1.4.0, 1.5.0
- Upgrade from 1.2.0 to any of 1.3.0, 1.4.0, 1.5.0
- Upgrade from 1.3.0 to any of 1.4.0, 1.5.0
...
```
#Upgrade to OpenEBS control plane components to desired version. Say 1.5.0
...
- If the default values seem appropriate, you can use the below commands to
  ...
  ```sh
  $ helm upgrade --reset-values <release name> stable/openebs --version 1.5.0
  ```
- If not, customize the values into your copy (say custom-values.yaml),
  ...
  ```sh
  $ helm upgrade <release name> stable/openebs --version 1.5.0 -f custom-values.yaml`
  ```
...
like cStor or Jiva, verify that OpenEBS Control plane is indeed in desired version**
  ...
  You can use the following command to verify components are in 1.5.0:
  ```sh
  $ kubectl get pods -n openebs -l openebs.io/version=1.5.0
  ```
...
upgraded to 1.5.0, the Storage Pools and Volumes (both jiva and cStor)
...
examples show upgrading from 1.0.0 to 1.5.0.
