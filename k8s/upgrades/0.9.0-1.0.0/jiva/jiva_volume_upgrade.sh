...
if [[ "$controller_version" != "$current_version" ]] && \
    ...
    echo "Current Target deployment $c_deploy_name version is not $current_version \
            or $target_upgrade_version";exit 1;
fi
...
if [[ "$replica_version" != "$current_version" ]] && \
    ...
    echo "Current Replica deployment $r_deploy_name version is not $current_version \
            or $target_upgrade_version";exit 1;
fi
...
if [[ "$controller_svc_version" != "$current_version" ]] && \
    ...
    echo "Current Target service $c_svc_name version is not $current_version \
            or $target_upgrade_version";exit 1;
fi
...
if [[ "$replica_version" != "$target_upgrade_version" ]]; then
    ...
    rc=$?; if [ $rc -ne 0 ]; then echo "Failed to patch the deployment $r_deploy_name \
            | Exit code: $rc"; exit; fi

    for r_rs in $(echo "$r_rs_old_list" | tr ":" " "); do
        kubectl delete rs "$r_rs" --namespace "$ns"
        rc=$?; if [ $rc -ne 0 ]; then echo "Failed to delete ReplicaSet $r_rs  \
                | Exit code: $rc"; exit; fi
    done
...
if [[ "$controller_version" != "$target_upgrade_version" ]]; then
    ...
    kubectl patch deployment  --namespace "$ns" "$c_deploy_name" -p "$(cat jiva-target-patch.json)"
    rc=$?; if [ $rc -ne 0 ]; then echo "Failed to patch deployment $c_deploy_name \
            | Exit code: $rc"; exit; fi

    kubectl delete rs "$c_rs_old" --namespace "$ns"
    rc=$?; if [ $rc -ne 0 ]; then echo "Failed to patch deployment $c_rs_old \
            | Exit code: $rc"; exit; fi
...
if [[ "$controller_svc_version" != "$target_upgrade_version" ]]; then
    ...
    kubectl patch service --namespace "$ns" "$c_svc_name" -p "$(cat jiva-target-svc-patch.json)"
    rc=$?; if [ $rc -ne 0 ]; then echo "Failed to patch the service $c_svc_name \
            | Exit code: $rc"; exit; fi
else
