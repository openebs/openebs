...
if [ $rc -eq 0 ]; then 
   ...
   if [ $POOL_PATH = "/var/openebs" ]; then
     ...
     echo " After completing upgrade process, you will need to re-apply your StoragePool"
...
if [ -z $OLDER_PVS ] || [ $OLDER_PVS -lt 2 ]; then 
    echo "There are no PVs that need to be upgraded to 0.8.1"
else
    echo "Found PVs that need to be upgraded to 0.8.1"
fi
