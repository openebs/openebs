    ...
    for i in boxlist:
        if i[0] != 'localhost':
            ...
            try:
                ...
                logging.info ("Unable to construct hosts file, received error:", e)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Take the hostfile, logging level and SSH details')
    ...
    if "ansible/roles/inventory/files" in current_path:
        ansible_path = os.path.dirname(os.path.dirname(os.path.dirname(current_path)))
        ...
        for i in fp.readlines():
            ...
            if tmp[0] != '\n' and "#" not in tmp[0]:
                ...
                try:
                    ...
                    logging.info ("Unable to parse input, failed with error %s", e) 
                    exit()
    ...
    for i in codes:
        ...
        generateInventory(config, codeSubList, inventory_path, SupportedHostCodes, passwdless)
