...
def setupSSH(key_rsa_path, key_append_path, key_gen_cmd, HostList):
    """ Setups up passwordless SSH between the localhost and other target hosts """
    ...
    for i in HostList:
        if i[0] != 'localhost':
            ...
            try:
                ...
                logging.info ("Passwordless SSH setup failed b/w localhost & %s with %s," \
                        "please verify host connectivity", box_ip, e)
