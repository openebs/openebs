...
def create_test_plan(client, args, storage_engine_type, project_id):
    plan_name = "BUILD_{}_{}_{}_{}".format(args['image_tag'], storage_engine_type, args['build_number'],
                                           datetime.now()[:10]
                                           )
    description = "Jenkins logs: http://104.197.185.168:8080/blue/organizations/jenkins/" \
                  "Jiva/detail/Jiva/{}/pipeline/\nUsername: test\nPassword:".format(args['build_number'])
    ...
    if suite_info is not None and 'Exclude' in suite_info and suite_info['Exclude'] is not None:
        ...
        for case in cases:
            if case['id'] in suite_info['Exclude']:
                ...
                ids.append(case['id'])
        suite = client.send_post('add_plan_entry/{}'.format(plan_id),
        ...
        suite = client.send_post('add_plan_entry/{}'.format(plan_id),
                                 {'suite_id': suite_id, 'description': description}
                                 )
        if suite['id'] is None:
            print("Adding suite failed")
            exit(-1)
...
def create_plan_resources(args):
    ...
    map_src_id, run_id, project_id, cstor_plan_resources, jiva_plan_resources = {'cStor_disk': {},
                                                                                 'cStor_sparse': {},
                                                                                 'jiva': {}
                                                                                 } \
        , 0, yaml_info['TestRailProjectID'], [], []
    ...
    if len(cstor_plan_resources) > 0:
        ...
        map_src_id["cstor_plan_disk_run_id"], map_src_id[
            "cstor_plan_sparse_run_id"], case_resources, cstor_test_disk_yaml, cstor_test_sparse_yaml = create_test_plan(
            client, args, "CSTOR_DISK", project_id), create_test_plan(client, args, "CSTOR_SPARSE",
                                                                      project_id), [], [], []
            ...
            for case_resource in case_resources:
                ...
                cstor_test_disk_yaml.append({'include': case_resource['path'],
                                             'vars': {'status_id': S(''), 'testname': S(''), 'flag': S(''),
                                                      'cflag': S(''), 'storage_engine': S('cStor'), 'status': S(''),
                                                      'cstor_sc': S('openebs-cstor-disk')
                                                      }})
                ...
                cstor_test_sparse_yaml.append({'include': case_resource['path'],
                                               'vars': {'status_id': S(''),
                                                        'testname': S(''), 'flag': S(''), 'cflag': S(''),
                                                        'storage_engine': S('cStor'), 'status': S(''),
                                                        'cstor_sc': S('openebs-cstor-sparse')
                                                        }
                                               })
        ...
        if len(case_resources) > 0:
            ...
            for case_resource in case_resources:
                ...
                jiva_test_yaml.append({'include': case_resource['path'],
                                       'vars': {'status_id': S(''), 'testname': S(''), 'flag': S(''), 'cflag': S(''),
                                                'storage_engine': S('jiva'), 'status': S('')}})
