...
func TestNewEnvClient(t *testing.T) {
	cases := []struct {
		...
		{
			envs: map[string]string{
				"OPENEBS_CERT_PATH": "invalid/path",
			},
		...
		},
		{
			...
			expectedError: "unable to parse openebs host `host`",
		},
		{
			envs: map[string]string{
				"OPENEBS_HOST": "invalid://url",
			},
			expectedVersion: DefaultVersion,
		},
		{
			envs: map[string]string{
				"OPENEBS_API_VERSION": "anything",
			},
			expectedVersion: "anything",
		},
		{
			envs: map[string]string{
				"OPENEBS_API_VERSION": "1.22",
			},
...
func TestGetAPIPath(t *testing.T) {
	...
	for _, cs := range cases {
		c, err := NewClient("unix:///var/run/openebs.sock", cs.v, nil, nil)
		if err != nil {
			t.Fatal(err)
		}
...
func TestNewEnvClientSetsDefaultVersion(t *testing.T) {
	...
	expected := "1.22"
	os.Setenv("OPENEBS_API_VERSION", expected)
	client, err = NewEnvClient()
	if err != nil {
		t.Fatal(err)
	}
